...
$resolutions   = array(2200, 1382, 768, 480); // the resolution break-points to use (screen widths, in pixels)
$cache_path    = "assets/img/ai-cache"; // where to store the generated re-sized images. Specify from your document root!
...
function sendErrorImage($message) {
  ...
  $influx_orange    = ImageColorAllocate($im, 255, 135, 50);
  $influx_fuschia    = ImageColorAllocate($im, 234, 24, 119);
  ...
  ImageString($im, 5, 5, 45, "The adaptive image resizing software failed.", $influx_orange);
  ImageString($im, 5, 5, 65, "An Influx employee with admin privileges for the server can fix this.", $influx_fuschia);
